...
export async function action({ request, context }: ActionFunctionArgs) {
    ...
    try {
        ...
        for (let i = startIndex; i < lines.length; i++) {
            ...
            try {
                if (type === "OUT") {
                    // OUT format: 日期,品牌,型号,数量,部门,经手人,备注
                    if (parts.length < 6) {
                        errors.push(`行 ${lineNumber}: 列数不足，需要至少6列`);
                        continue;
                    }
                    ...
                    const handlerName = parts[5].trim();
                    const note = parts.length > 6 ? parts[6].trim() : "";
                    ...
                    if (!product) {
                        if (autoCreate && categoryId) {
                            ...
                            errors.push(`行 ${lineNumber}: 找不到产品 "${brand} ${model}"${autoCreate ? "，且未选择分类" : ""}`);
                            continue;
                        }
                    }
                    ...
                    // IN format: 日期,品牌,型号,数量,单价,供应商,备注
                    if (parts.length < 6) {
                        errors.push(`行 ${lineNumber}: 列数不足，需要至少6列`);
                        continue;
                    }
                    ...
                    const model = parts[2].trim();
                    const quantityStr = parts[3].trim();
                    const priceStr = parts[4].trim();
                    const supplier = parts[5].trim();
                    const note = parts.length > 6 ? parts[6].trim() : "";
        ...
        if (newProducts.length > 0) {
            for (const np of newProducts) {
                ...
                ).bind(np.category_id, np.brand, np.model, np.price || 0).run();
...
function generateCSVTemplate(type: string): string {
    if (type === "OUT") {
        return "日期,品牌,型号,数量,部门,经手人,备注\n2024-01-15,Kingston,USB 3.0 32GB,10,技术部,张三,批量采购\n2024-02-20,SanDisk,移动硬盘1TB,5,市场部,李四,";
    } else {
        return "日期,品牌,型号,数量,单价,供应商,备注\n2024-01-10,Kingston,USB 3.0 32GB,100,45.50,京东,年度采购\n2024-02-15,SanDisk,移动硬盘1TB,50,320.00,天猫,季度采购";
    }
}
            ...
            <div className="glass-panel" style={{ padding: "2rem", maxWidth: "800px", margin: "0 auto" }}>
                ...
                <div style={{ marginBottom: "2rem", padding: "1.5rem", background: "var(--bg-secondary)", borderRadius: "var(--radius-sm)", border: "1px solid var(--border-light)" }}>
                    ...
                    <p style={{ fontSize: "0.875rem", marginBottom: "1rem", color: "var(--text-secondary)", lineHeight: "1.6" }}>
                        {currentType === "OUT"
                            ? "每行包含：日期、品牌、型号、数量、部门、经手人、备注（可选）"
                            : "每行包含：日期、品牌、型号、数量、单价、供应商、备注（可选）"
                        }
                    </p>
                    <button
                ...
                <Form method="post" encType="multipart/form-data" style={{ display: "grid", gap: "1.5rem" }}>
                    ...
                    <button type="submit" className="btn btn-primary" disabled={isSubmitting || (autoCreate && !selectedCategory)}>
