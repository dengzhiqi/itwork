...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const { results: staff } = await env.DB.prepare(
    "SELECT * FROM staff ORDER BY department, name ASC"
  ).all();
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (intent === "rename_dept") {
    ...
    await env.DB.prepare("UPDATE departments SET name = ? WHERE name = ?")
      .bind(newName, oldName)
      .run();

    // Update references in staff table
    await env.DB.prepare("UPDATE staff SET department = ? WHERE department = ?")
      .bind(newName, oldName)
      .run();
  ...
  if (intent === "delete_dept") {
    ...
    await env.DB.prepare("DELETE FROM departments WHERE name = ?")
      .bind(deptName)
      .run();
  ...
  if (intent === "add_dept") {
    ...
    await env.DB.prepare("INSERT INTO departments (name) VALUES (?)")
      .bind(deptName)
      .run();
  ...
  if (intent === "add_supplier") {
    ...
    await env.DB.prepare(
      ...
      .bind(company_name, contact_person, phone, email)
      .run();
...
export default function Settings() {
  const { categories, staff, departments, suppliers, user } =
  ...
  const isAdding = navigation.formData
    ?.get("intent")
    ?.toString()
    .startsWith("add");
                    ...
                    {cat.product_count === 0 && (
                      ...
                      >
                        ...
                        <button
                          ...
                          title="删除"
                          onClick={(e) =>
                            !confirm("确定要删除吗?") && e.preventDefault()
                          }
                        >
                          ✕
                        </button>
              ...
              >
                ...
                <tbody>
                  {staff.map((s: any, index: number) => {
                    ...
                    const isFirstInDept =
                      index === 0 ||
                      staff[index - 1].department !== s.department;
        ...
        {activeTab === "system" && (
          <div className="glass-panel" style={{ padding: "2rem" }}>
            ...
            {showCustomEditor && (
              ...
              >
                <h4 style={{ fontSize: "1.125rem", marginBottom: "1.5rem" }}>
                  自定义主题颜色
                </h4>
                ...
                >
                  ...
                  <ColorPicker
                    ...
                    onChange={(value) =>
                      handleCustomColorChange("primaryColor1", value)
                    }
                  />
                  <ColorPicker
                    label="主色调 2 (渐变结束)"
                    ...
                    onChange={(value) =>
                      handleCustomColorChange("primaryColor2", value)
                    }
                  />
                  <ColorPicker
                    label="大标题颜色"
                    ...
                    onChange={(value) =>
                      handleCustomColorChange("headingColor", value)
                    }
                  />
                  <ColorPicker
                    label="正文颜色"
